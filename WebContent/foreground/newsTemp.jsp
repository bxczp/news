					...
					<div class="datas">
						<ul>
							<c:forEach var="newNews" items="${newNewsList }">
								<li><a href="news?action=show&newsId=${newNews.newsId }"
									title="${newNews.title }">
										${fn:substring(newNews.title,0,22)} </a></li>
							</c:forEach>
						</ul>
					</div>
				</div>
					...
					<div class="datas">
						<ul>
							<c:forEach var="hotNews" items="${hotNewsList }">
								<li><a href="news?action=show&newsId=${hotNews.newsId }"
									title="${hotNews.title }">
										${fn:substring(hotNews.title,0,22)} </a></li>
							</c:forEach>
						</ul>
					</div>
				</div>
